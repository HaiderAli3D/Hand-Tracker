...
def process_right_hand(hand_landmarks, frame, frameWidth, frameHeight):
    ...
    if global_hand_state:
        # Left click
        if distances["index"] < 0.058 and not right_hand_states["mouseDown"] and not right_hand_states["middleMouseDown"]:
            pag.mouseDown()
            right_hand_states["mouseDown"] = True
        if distances["index"] > 0.085 and right_hand_states["mouseDown"]:
            pag.mouseUp()
        ...
        if distances["middle"] > 0.85 and right_hand_states["rightMouseDown"]:
